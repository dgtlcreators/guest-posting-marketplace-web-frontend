    ...
    useEffect(() => {
        const fetchData=async()=>{
            try {
               ...
               const response= await axios.get(`${localhosturl}/reportroute/getallreports`);
...
fetchData()
    ...
    const deleteUser = async (userId) => {
        try {
            const response = await axios.delete( `${localhosturl}/reportroute/deletereport/${userId}`);
                    ...
                    <div className='overflow-x-auto  p-2 rounded-lg shadow-md'>
                        <table className="min-w-full  border border-gray-300">
                            <thead>
                                ...
                                <th className="border py-3 px-2 md:px-6 text-left" onClick={() => handleSort("reportType")}>Report Type {renderSortIcon("reportType")}</th>
                                <th className="border py-3 px-2 md:px-6 text-left" onClick={() => handleSort("reason")}>Reason {renderSortIcon("reason")}</th>
                                <th className="border py-3 px-2 md:px-6 text-left" onClick={() => handleSort("details")}>Details {renderSortIcon("details")}</th>
